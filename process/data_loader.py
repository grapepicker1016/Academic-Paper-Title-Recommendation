...
def load_and_prepare_data_for_t5(data_path, test_size=0.2, random_state=101):
    df = pd.read_csv(data_path)
    train_df, eval_df = train_test_split(df, test_size=test_size, random_state=random_state)
    return train_df, eval_df

def preprocess_data_for_t5(df):
    df['prefix'] = "summarize"
    return df

def create_df_from_json(path_to_json):
...
def fit_text_for_lstm(X, Y, input_seq_max_length=None, target_seq_max_length=None):
